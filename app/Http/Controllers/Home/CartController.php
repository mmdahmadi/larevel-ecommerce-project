...
class CartController extends Controller
{
    public function add(Request $request)
    {
        ...
        if ($request->qtybutton > $productVariation->quantity){
            alert()->warning('تعداد وارد شده از موجودی بیشتر است' , 'دقت کنید');
    ...
    {
        \Cart::remove($rowId);
        alert()->success('محصول مورد نظر از سبد خرید شما حذف شد' , 'دقت کنید');
        return redirect()->back();
    }
    public function clear()
    {
        \Cart::clear();
        alert()->warning('از سبد خرید شما پاک شد' , 'دقت کنید');
        return redirect()->back();
    }
    ...
    public function checkCoupon(Request $request)
    {
        ...
        if (!auth()->check()){
            alert()->error('برای استفاده از کد تخفیف نیاز است ابتدا وارد سایت شوید' , 'دقت کنید');
            return redirect()->back();
        }
